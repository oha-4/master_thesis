...
\section{離散制御器合成における計算空間の削減手法}

離散制御器合成の実適用における最大の課題は，システム規模の増大に伴い状態数が指数関数的に増加する状態空間爆発問題である．
この問題に対処するため，全状態空間の単純な列挙を回避し，計算空間を削減するためのアプローチが数多く提案されている．
これらは主に，システムの構造を利用して問題を分割する手法，抽象化を用いてモデルを縮約する手法，および状態空間の表現や構築方法自体を効率化する手法に大別される．

システムのモジュール構造に着目した初期のアプローチとして，WonhamとRamadgeはモジュラー制御（Modular Supervisory Control）を提案した~\cite{ModularSupervisoryControlOfDiscreteEventSystems}．
...
これは，システムを抽象化した上位層にコーディネータを配置し，大域的な視点から下位層の制御器間の調停を行うことで，計算効率と解の整合性を両立させる試みである．
...
具体的には，Mohajeraniらは，合成の各段階において制御に必要な情報を保持しつつモデルを簡略化する合成抽象化（Synthesis Abstraction）の概念を導入し，最小制限（Least Restrictive）性を保持したまま大規模システムに対する合成を可能とする枠組みを示した~\cite{CompositionalSynthesisOfDiscreteEventSystemsUsingSynthesisAbstraction}．
また，山内らは，要求仕様に基づいて必要最小限のモデル構造を段階的に構築しつつ部分合成を行う手法を提案し，充足不可能な状態を早期に剪定することで分析空間を削減している~\cite{StepwisePartialSynthesis}．

一方，状態空間の表現形式や構築プロセス自体を工夫することで計算資源を節約するアプローチも存在する．
Thuijsmanらは，二分決定グラフ（Binary Decision Diagram, BDD）を用いたシンボリックな合成手法において，変数の順序付けや演算順序を最適化するヒューリスティックを提案し，明示的な状態列挙を行わずに大規模な論理空間を効率的に処理する手法を示した~\cite{ReducingTheComputationalEffortOfSymbolicSupervisorSynthesis}．
また，山内らは，環境モデルと監視モデルから全体モデル（LTS）を構築することなく，ゲーム空間を直接構築するConsolidated Discrete Controller Synthesis（CDCS）を提案した~\cite{ConsolidatedDiscreteControllerSynthesis}．
CDCSは，安全性要件に違反する状態が生成されないよう制御ゲーム空間を構成することにより，大幅な計算空間削減を達成している．

これらの手法は，モデルの構造や定義を工夫することで計算対象となる空間自体を削減する有効なアプローチである．
しかし，これらの手法は，構造分割，抽象化，あるいはモデル表現の工夫といった設計時の前処理を前提とする場合が多い．
これに対し，探索アルゴリズムそのものの挙動を制御し，必要な経路のみを動的に展開するOn-The-Fly探索のアプローチが存在する．

\section{On-The-Fly探索を用いた計算空間削減}

離散事象システムやゲーム理論に基づく検証・合成問題において，状態空間爆発は回避すべき最大の課題である．
これに対し，全状態空間を事前に構築することなく，初期状態から到達可能な必要な部分空間のみを動的に展開するOn-The-Fly探索が有効なアプローチとして確立されている．
Cassezら~\cite{OnTheFlyForTimedGames}は，Timed Gamesにおいて，従来主流であった後方固定点計算に基づく解法ではなく，前方探索（Forward Search）により状態空間を動的に展開するOn-The-Flyアルゴリズムを提案した．
...
Miremadiら~\cite{SymbolicOnTheFlySynthesis}は，スーパバイザ制御理論（SCT）において，二分決定グラフ（BDD）を用いたシンボリックな状態表現とOn-The-Fly探索を組み合わせた合成手法を提案した．
彼らは，AIプランニングの探索技術に着想を得た前方探索を用いることで，制御不能状態やブロッキング状態を早期に検出し，不要な探索を抑制することで，大規模システムに対する合成の実用性を示している．

これらの前方探索に基づく合成手法において，探索効率を大きく左右する要因の一つが，次にどの遷移を展開すべきかを決定する探索指針，すなわちヒューリスティック関数の設計である．
適切なヒューリスティックを用いれば，解に繋がる有望な経路を優先的に探索することで計算コストを劇的に削減できる一方，不適切な指針は探索空間の無用な拡大を招く．
次節では，DCSの効率化を目的とした探索ヒューリスティックに関する既存研究と，それらが抱える課題について詳述する．

\section{On-The-Fly探索におけるヒューリスティック}

On-The-Fly探索において，次にどの遷移を展開するかを決定するヒューリスティック関数の設計は，探索効率を決定づける最も重要な要素である．
適切なヒューリスティックを用いることで，解に繋がる有望な経路を優先的に探索し，計算コストを大幅に削減することが可能となる．
本節では，DCSのために提案された代表的なヒューリスティック探索手法について概説する．

Ciolekらは，離散事象システムの並列合成構造に着目し，ドメイン独立なヒューリスティックであるReady Abstraction（RA）を提案した~\cite{ReadyAbstraction}．
RAは，各コンポーネントにおいて局所的に発火可能なアクション（Readyアクション）を抽出し，それらの間の遷移コストに基づいて目標までの距離を推定する手法である．
この手法は，コンポーネントごとの局所的な距離情報と，アクション間の依存関係を組み合わせることで，全状態空間を構築することなく大域的な距離を見積もることを可能にした．
RAの導入により，DCSの適用範囲は拡大し，従来の手法では扱えなかった規模のシステムに対しても制御器の合成が可能となったことが報告されている~\cite{ReadyAbstraction}．

また，人手によって設計されたヒューリスティックに加え，近年では機械学習を用いて探索戦略を自動的に獲得する試みもなされている．
Delgadoらは，強化学習（Reinforcement Learning）をDCSの探索プロセスに適用し，遷移展開の順序を決定する探索ポリシーを学習する手法を提案した~\cite{ReinforcementOnTheFly}．
彼らは，探索過程の状態や選択可能な遷移を特徴量空間に抽象化し，探索に要するステップ数を最小化するような方策をニューラルネットワークにより学習させた．
この手法は，小規模な問題インスタンスで学習したモデルを大規模なインスタンスへ適用（ゼロショット転移）することで，特定の問題設定においてはRAを上回る探索効率を実現できることを示している~\cite{ReinforcementOnTheFly}．

しかし，これらの先行研究には依然として課題が残されている．
RAは，Readyアクション間の距離に基づく推定を行うため，コンポーネント間の同期に伴う複雑な待機時間や，将来的に不可避となるデッドロックの構造を正確に予測しきれない場合がある．
その結果，局所的には最適に見えるが実際には目標に到達できない経路を過剰に探索してしまうという問題が生じうる．
一方で，強化学習に基づく手法は，探索ポリシーの獲得に多大なトレーニングコストを要する上，学習データの質や問題の構造に性能が依存する可能性がある．

したがって，より効率的かつ安定したDCSを実現するためには，事前の学習を必要とせず，かつシステムの大域的な構造情報やボトルネックを直接的に探索指針として組み込むことのできる，新たなヒューリスティックの枠組みが求められている．
