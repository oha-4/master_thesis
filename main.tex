...
提案手法の有効性を検証するために実施したベンチマーク実験の結果を述べる．

\subsection{実験設定}
% 使用したベンチマーク問題，環境，比較対象など

\subsection{実験結果}
% 探索状態数，計算時間，PMAの個数などの表・グラフ

\section{考察と適用戦略}
\label{sec:discussion}

実験結果に基づき，PMDの特性と適用範囲について考察する．

\subsection{PMDの有効性と限界}
% PMAが多く検出される問題では圧倒的に早いが，PMAがほぼ検出できない問題ではRAに劣る場合があること（結果からわかること）

\subsection{RAとの動的切り替え戦略}
% 先にPMD（PMA算出）を試み，その結果によってRAと使い分ける戦略
% PMA算出コスト（数百ms）が探索全体に比べて軽微であるため，まずはPMAを計算し，PMA数が十分であればPMD，そうでなければRAを選択するというハイブリッドな戦略が有効である点について論じる．
