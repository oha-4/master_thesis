...
離散制御器合成（DCS）の実適用における最大の課題は，システム規模の増大に伴い状態数が指数関数的に増加する状態空間爆発問題である．
...
本手法は，単一の巨大な全体仕様を複数の部分仕様に分割し，各仕様に対する制御器を個別に合成することで，合成計算の複雑さをシステム規模の積から和のオーダーへと低減させる．
また，WongとWonhamは，モジュラー制御において局所的な制御器間の競合により発生するブロッキングを解決するため，階層的制御（Hierarchical Control）を導入した~\cite{ModularControlAndCoordinationOfDiscreteEventSystems}．
...
具体的には，Mohajeraniらは，合成の各段階において制御に必要な情報を保持しつつモデルを簡略化する合成抽象化（Synthesis Abstraction）の概念を導入し，大規模システムに対する最小制限スーパバイザの合成を実現した~\cite{CompositionalSynthesisOfDiscreteEventSystemsUsingSynthesisAbstraction}．
また，山内らは，要求仕様に関連する環境モデルのみを抽出して段階的に部分合成を行う手法を提案し，充足不可能な状態を早期に剪定することで分析空間を削減している~\cite{StepwisePartialSynthesis}．
...
CDCSは，安全性要件に違反する状態の生成そのものを回避するようゲーム空間を定義することで，計算空間を最大で99\%削減することに成功している．

これらの手法は，モデルの構造や定義を工夫することで計算対象となる空間自体を削減する有効なアプローチである．
しかし，これらの多くは事前に何らかの構造解析や抽象化モデルの構築を必要とするか，あるいは特定のモデル構造を前提としている．
