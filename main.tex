...
\aref{alg:ready_abstraction}は，現在の合成状態 $\bm{s}$ において展開対象のアクション $\hat{a}$ を選ぶことがどれだけ目標に近いかを評価するためのヒューリスティック関数である．
...
\aref{alg:ready_abstraction}の戻り値は各コンポーネントの距離推定の組であるが，実際の探索（\textsc{ExpandNext}）においてアクション $\hat{a}$ を選択する際は，以下の優先順位に従って順位付けを行う：
...
入力は現在の合成状態 $\bm{s}$，次に実行するアクション $\hat{a}$，距離の対象となるコンポーネント状態 $s^*_{E_i}$ であり，出力は推定距離 $d$ である．
...
\begin{description}
  ...
  RAは，Readyアクションのうち展開可能なアクション「研磨」，「皮膜」，「溶解」について，目標（Marked状態）までの距離を推定する．
  ...
  \begin{description}
    ...
    $E_M$ が状態1（Marked）へ遷移するためには，同期アクションである「出荷」の発火が不可欠である．
    \aref{alg:estimate_dist}における距離推定値 $d$ は，展開対象のアクション $\hat{a}$ から「出荷」アクション $a_m$ への切り替えコストである $\textsc{CalculateGap}(\bm{s}_1, \hat{a}, a_m)$ と，そこからMarked状態へ到達する距離（ここでは1）の和として計算される．
    \begin{description}
      \item[研磨・皮膜] $E_\mathit{env}$ において，「研磨」または「皮膜」から「出荷」へのGap（遷移コスト）は1である．したがって，Marked状態への距離推定値 $d$ は，$1 + 1 = 2$ と評価される．
      \item[溶解] $E_\mathit{env}$ において，「溶解」から「成形」「冷却」を経て「出荷」に至る最小のGapは3である．したがって，Marked状態への距離推定値 $d$ は，$3 + 1 = 4$ と評価される．
    \end{description}
  \end{description}
