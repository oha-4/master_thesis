...
\begin{description}
  ...
  \descitem{多重な距離推定に伴う計算コストの増大}
  RAはヒューリスティック値を算出する際，全てのコンポーネントに対し，既知および未知のMarked状態への到達距離を個別に推定する．この網羅的な評価プロセスは，システム規模（コンポーネント数）に比例して重い距離計算を繰り返すことを意味し，大規模なシステムにおいて1ステップごとの計算時間を増大させる主要因となる．
\end{description}
...
\begin{description}
  ...
  これにより，近視眼的な探索による迷走を防ぎ，探索状態数を大幅に削減することで，システム全体の合成効率を向上させる．
...
形式的な定義を与えるために，まず目標への到達トレース集合を定義する．
LTS $E = (S_E, s_{E,0}, S_E^I, A_E, \Delta_E)$ において，状態 $s \in S_E$ を始点とし，目標アクション集合 $A^\mathit{Target} \subseteq A_E$ の発火をもって終了する有効な有限トレースの集合を $T_{s \to A_E^\mathit{Target}}$ とする．

$$
  T_{s \to A^\mathit{Target}} = \left\{
  t = s_0, a_0, s_1 ,\dots, a_{n-1}, s_n \in T_E \;\middle|\;
  \begin{aligned}
     & s_0 = s,                                         \\
     & \forall i \in \{0, \dots, n\}, s_i \notin S_E^I, \\
     & a_{n-1} \in A^\mathit{Target}
  \end{aligned}
  \right\}
$$

この $T_{s \to A_E^\mathit{Target}}$ を用いて，必須アクション集合 $M^R_E$ を以下のように定義する．
...
本研究では，これらを総称してPre-Markingアクションと呼び，探索の指針として用いる．

\section{Pre-Markingアクションの抽出アルゴリズム}

前節の定義に基づき，各状態におけるPre-Markingアクションを抽出するアルゴリズムについて述べる．
...
なお，本アルゴリズムはPre-Markingアクションの抽出を目的としているため，初期状態において必須でないアクションは，各状態における必須性に関わらず除外する．
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \For{$\hat{a} \in \hat{A}_{E}$}
      \LComment{アクション $\hat{a}$ を通らずにターゲットに到達可能な状態（不要状態）を探索}
...
第一段階では，Markingアクション集合 $A^M$ を目標としてPrimary Pre-Markingアクション $M^\mathit{PP}_E$ を抽出する．
...
第二段階では，抽出されたPrimary Pre-Markingアクションを起点として，Secondary Pre-Markingアクション $M^\mathit{SP}_E$ の抽出を行う．
ここでは，あるコンポーネントで発見された必須アクションが，同期を介して他のコンポーネントにおける必須アクションとなる依存関係を網羅する必要がある．
そのために，不動点反復に基づくアプローチを採用する．
具体的には，初期状態で有効なPre-Markingアクションを探索すべき目標集合として管理し，未処理の目標に対して各LTSで必須アクション抽出を実行する．
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \State $A^* \gets \bigcup_{i=1}^{n} M^\mathit{PP}_{E_i}(s_{E_i,0})$ \Comment{初期状態で有効なPrimary Pre-Markingアクションが初期目標}
    ...
    \While{$A^*_\mathit{new} \neq \emptyset$} \Comment{新たな目標が追加されなくなるまで反復}
      \ForAll{$a^* \in A^*_\mathit{new}$} \Comment{未処理の目標アクションについて}
        ...
        \EndFor
      \EndFor
      \LComment{新たに発見されたPre-Markingアクションを次回の目標とする}
...
本節では，抽出されたPre-Markingアクションを用い，各状態の有望さを定量的に評価するヒューリスティック関数について述べる．
...
これに対しLandmark Ready Abstraction (LRA) では，抽出されたPre-Markingアクションを，最終目標に至るために経由しなければならない中間目標（ランドマーク）として扱う．
未解決のPre-Markingアクションを距離計算の対象に含めることで，ボトルネックの解消にかかるコストを評価値に反映し，より精度の高い距離推定を実現する．

\subsection{ヒューリスティック関数の構成}

LRAにおけるヒューリスティック関数 $\textsc{LandmarkReadyAbstractionHeuristic}$ の全体構成について述べる．
本関数は，最終目標であるMarkingアクションへの到達距離を基準としつつ，抽出されたPre-Markingアクションへの距離を評価に組み込むことで，構造的なボトルネックを反映した値を算出する．

具体的な計算手順は以下の通りである．
まず，基本となる評価値として，システムの最終目標であるMarkingアクション集合 $A^M$ への到達距離を算出する．
これにより，障害物がない理想的な状況下での最短ステップ数が得られる．

次に，現在の状態において解決すべきPre-Markingアクション集合 $A^P$ を構築する．
この集合は，現在の状態で必須となっているPrimary Pre-Markingアクションを起点とし，そこから依存関係にあるSecondary Pre-Markingアクションを再帰的に探索することで生成される．
具体的には，Primary Pre-Markingアクション集合に対し，各アクションの前提となるSecondary Pre-Markingアクションを追加する操作を，新たなアクションが追加されなくなるまで繰り返す．
これにより，Primary層だけでなく，より深い階層にある潜在的なボトルネックも網羅的に収集される．

最後に，収集された各Pre-Markingアクション $a \in A^P$ について到達距離を算出し，これらを現在の推定距離と比較して最大値を更新する．
ここで，Pre-Markingアクションへの距離に $1$ を加算しているのは，Pre-Markingアクションが最終目標に至るための通過点であり，その達成後も少なくとも1ステップ以上の遷移が必要であることを評価値に反映させるためである．
以上の手順により，最終目標への距離と，回避不可能な中間目標への距離のうち，最も解決に手数を要するものがヒューリスティック値として採用される．
...
RAにおける距離推定（\aref{alg:estimate_dist_to_states}）が，目標となる状態集合への到達距離を算出するのに対し，本関数は目標となるアクション集合への到達距離を算出する点が異なる．
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \For{$i = 1, \ldots, n$}
      \State $s' \gets s \text{ where } (s_{E_i}, \hat{a}, s) \in \Delta_{E_i}$
      \If{$s' \neq \bot \land s' \notin S^I_{E_i}$}
        \State $A^* \gets A^* \cup \{ a^* \in A^R \cup A^\mathit{Target} \mid T_{s' \to \{a^*\}} \neq \emptyset \}$
      \EndIf
    \EndFor
...
RAが陥った局所解への迷走を，LRAがいかにして回避し，同期に不可欠なアクションを見据えた適切な探索を行うかについて詳述する．
...
RAは目標（「出荷」）への局所的な近さのみを見て「研磨」（距離2）を推奨したが，LRAはシステム全体としてPMA（「成形」）への到達が困難になるペナルティを統合された評価値に反映している．
これにより，LRAは見かけ上の近道である研磨ルートを却下し，必須アクションを確実に消化できる「溶解」ルートを正しく選択する．
