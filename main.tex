...
現代の社会インフラを支える複雑な離散事象システムにおいて，要求仕様を満たす制御ロジックを自動生成する離散制御器合成（Discrete Controller Synthesis）の重要性が高まっている．
離散制御器合成の実用化における最大の障壁は，コンポーネント数の増加に伴い状態空間が指数関数的に増大する状態空間爆発問題である．
これに対し，既存のReady Abstractionなどのヒューリスティックを用いたOn-The-Fly探索が提案されているが，これらは局所的な遷移可能性のみに依存するため，大規模システムにおける複雑な同期構造を十分に考慮できず，探索効率が著しく低下するという課題があった．

本研究では，目標達成のために構造上回避不可能なアクションをPre-Markingアクションとして定義し，これらを探索の中間目標（ランドマーク）として利用する新たなヒューリスティック手法「Landmark Ready Abstraction（LRA）」を提案した．
提案手法は，事前解析によってシステム全体の依存関係を階層的に抽出し，現在の状態で解決すべきボトルネックまでの距離を動的に算出することで，同期制約を考慮した大域的な経路選択を実現する．
また，評価対象を不可避なアクションに厳選することで，1ステップあたりの計算コストの削減も図る．

提案手法を実装し，複数のベンチマーク問題を用いて評価実験を行った結果，LRAは従来手法と比較して探索状態数を大幅に削減し，特に複雑な同期を要するシナリオにおいて極めて高い探索効率を示した．
また，コンポーネント数が増大した環境においても，ヒューリスティックの計算負荷を低く抑えることで，合成時間を最大で300倍以上短縮することに成功した．
以上の成果により，提案手法が大規模かつ複雑な離散事象システムに対する制御器合成の効率化において極めて有効であることを実証した．
...
\aref{chap:background}「背景技術」では，本研究の基礎となる離散制御器合成およびDirected Controller Synthesis（DCS）の理論的枠組み，On-The-Fly探索アルゴリズムについて概説する．
また，既存のヒューリスティック手法であるReady Abstraction（RA）の動作原理について詳述する．

\aref{chap:ready_abstraction_limitations}「Ready Abstractionの課題」では，既存のRAが抱える構造的な限界を分析する．
具体的には，金属加工システムのモデルを用いたケーススタディを通じ，Markingアクションの発火に同期が必要な状況において，RAの局所的な推定が大域的な同期制約を考慮できず，冗長な経路探索やバックトラックを招くメカニズムを明らかにする．
また，システム規模に応じた計算コストの増大についても論じる．

\aref{chap:landmark_ready_abstraction}「Landmark Ready Abstraction」では，本研究の提案手法について詳述する．
まず，目標達成に不可欠な「Pre-Markingアクション（PMA）」を定義し，これらを静的解析によって抽出・階層化するアルゴリズムを提案する．
次に，抽出されたPMAを中間目標（ランドマーク）として統合した新たなヒューリスティック関数を定義し，提案手法がいかにして同期ボトルネックを回避した経路選択を行うか，その動作原理を解説する．

\aref{chap:evaluation}「評価」では，提案手法の有効性を検証するための評価実験の結果について述べる．
複数のベンチマーク問題を用いた比較実験により，探索状態数および計算時間の観点から従来手法に対する優位性を定量的に評価する．
さらに，PMAの占有率と探索効率の相関に基づき，提案手法が特に有効に機能するシステムの特性について考察する．

\aref{chap:related_works}「関連研究」では，離散事象システムの制御器合成における状態空間爆発問題への各種アプローチや，AIプランニングにおけるランドマーク技術の動向を概観し，本研究の位置づけを明確にする．

\aref{chap:conclusion}「結論」では，本論文の成果を総括し，本手法の適用限界や今後の拡張性といった将来の研究展望について述べる．
...
RAはこの他コンポーネントの同期待ちという大域的な制約を考慮できず，各コンポーネントが局所的な最短経路を選択し続けるような局所最適化に基づいた評価を下すため，結果として合流不可能な経路や，効率の悪い状態を優先的に探索する結果となる．
...
すなわち，アクションを1つ選択するたびに，最大で $2n$ 回の距離推定関数（\textsc{Estimate\-Dist\-To\-State}）が呼び出されることになる．
...
本実験の目的は，\aref{chap:ready_abstraction_limitations}で指摘したReady Abstraction（RA）の課題である，同期構造の看過による探索空間の拡大および網羅的な距離推定による計算資源の浪費が，提案手法によって解決されているかを検証することである．
...
\begin{table}
  ...
  \begin{rtblr}{cell{2,6,10}{1} = {r=4}{c}}
    ...
    \midrule
    $(4, 5)$
     & BFS & 6025  & ( & 12.8 & ) & 995.75  & ( & 8.5 & ) & 21311   & ( & 147.0  & ) & \NA   &   &      &   \\
     & RA  & 485   & ( & 1.0  & ) & 132.56  & ( & 1.1 & ) & 516     & ( & 3.6    & ) & 382   & ( & 3.3  & ) \\
     & SRA & 472   & ( & 1.0  & ) & 116.63  & ( & 1.0 & ) & 136     & ( & 0.9    & ) & 115   & ( & 1.0  & ) \\
     & LRA & 472   & ( & 1.0  & ) & 116.67  & ( & 1.0 & ) & 145     & ( & 1.0    & ) & 117   & ( & 1.0  & ) \\
    \cmidrule[lr]{1-Z}
    $(5, 5)$
    ...
    $(5, 10)$
     & BFS & \NA   &   &      &   & \NA     &   &     &   & \TO     &   &        &   & \NA   &   &      &   \\
     & RA  & 2756  & ( & 2.4  & ) & 711.16  & ( & 1.0 & ) & 15885   & ( & 15.3   & ) & 11931 & ( & 13.3 & ) \\
     & SRA & 1134  & ( & 1.0  & ) & 695.20  & ( & 1.0 & ) & 1030    & ( & 1.0    & ) & 877   & ( & 1.0  & ) \\
     & LRA & 1134  & ( & 1.0  & ) & 695.23  & ( & 1.0 & ) & 1039    & ( & 1.0    & ) & 895   & ( & 1.0  & ) \\
    \bottomrule
  \end{rtblr}
\end{table}
...
RAの合成時間 $T$ はLRAと比較して大幅に長く，特にケース$(5, 10)$では約15倍の時間を要している．
...
実際，TLシナリオのケース$(20, 20)$において，LRAはRAと比較して評価時間を約100倍の高速化している．
...
本項では，本実験の結果および結論に対する潜在的な脅威について，内的妥当性と外的妥当性の観点から議論する．

\subsubsection{内的妥当性}

内的妥当性への脅威として，実装の正確性および計測環境の不確実性が挙げられる．

第一に，提案手法および比較手法の実装におけるバグが実験結果に影響を与えた可能性について述べる．
本実験では，Java言語で記述された既存のツールであるMTSAを拡張する形で各アルゴリズムを実装した．
実装の正確性を担保するため，比較的小規模なベンチマーク問題において，全探索（BFS），既存手法（RA），および提案手法（LRA）を実行し，全ての手法において制御器合成の成否の結果が完全に一致することを確認した．
また，LRAがPre-Markingアクション（PMA）を抽出できない場合において，その探索挙動および計算コストが静的RA（SRA）と完全に一致することを確認しており，アルゴリズムが意図通りに動作していることを検証済みである．

第二に，実行環境およびJava仮想マシン（JVM）の挙動に起因する計測誤差について述べる．
Java実行環境におけるガベージコレクションやJITコンパイル，およびOSのバックグラウンドプロセスは，実行時間計測に変動をもたらす要因となる．
本実験では，各アルゴリズムに対し十分な最大ヒープ領域（80GB）を割り当てることで頻繁なガベージコレクションの発生を抑制した．
また，計測値のばらつきを最小化するため，実験には他の負荷がかかっていない専用の計算機を用い，同一条件下での比較を行った．
各手法間の性能差は，多くのケースにおいて数倍から数百倍のオーダーで確認されており，微細な計測誤差が結論の優位性を覆す可能性は低いと考えられる．

\subsubsection{外的妥当性}

外的妥当性への脅威として，ベンチマーク問題の選定および現実のシステムへの一般化可能性が挙げられる．

第一に，使用したベンチマーク問題の代表性について述べる．
本実験では，離散制御器合成の研究において標準的に用いられる6つのシナリオに加え，より複雑な調整を要する2つのシナリオを採用した．
これらは多様な構造的特性を網羅しているものの，あくまで学術的な評価用に設計されたモデルであり，実産業における大規模システムと比較して構造が単純化されている可能性がある．
しかし，これらのモデルは並行システムにおけるリソース競合，同期，順序制約といった本質的な課題を含んでおり，提案手法の基本性能を評価する上で十分な複雑さを有していると判断できる．

第二に，提案手法の有効性がシステムの構造的特性に依存する点について述べる．
本手法は，システム内に存在する構造的なボトルネックを探索の指針として利用するアプローチである．
したがって，ボトルネックが極めて少ないフラットな構造を持つシステムや，各コンポーネント間の結合度が低いシステムにおいては，探索空間の削減効果が限定的となる可能性がある．
ただし，実用的な制御対象となるシステムの多くは，安全性や機能要件を満たすために特定の手順や同期を必要とするため，提案手法が有効に機能する構造的特徴を有している可能性は高いと考えられる．
...
本章では，離散制御器合成（Discrete Controller Synthesis）における状態空間爆発問題への対処手法の変遷を概観し，本研究の提案手法であるLandmark Ready Abstraction（LRA）の技術的基盤と位置づけを明確にする．
まず，基礎となるスーパバイザ制御理論と大規模システムへの適用における課題を述べ，その解決策として発展してきた静的なモデル縮約手法および動的なOn-The-Fly探索手法について，それぞれの代表的な研究を挙げて解説する．
次に，本研究が対象とするDirected Controller Synthesis（DCS）において，探索効率を決定づけるヒューリスティック技術に関する既存研究の成果と限界を論じる．
最後に，他分野であるAIプランニングにおいて成功を収めているランドマーク技術について触れ，本研究のアプローチとの関連性を示す．

\section{スーパバイザ制御理論と大規模システムへの展開}
...
これらの性質を，非同期に発生する離散的な事象の列としてモデル化し，制御理論の枠組みで形式的に扱ったのが，RamadgeとWonhamによって提唱されたスーパバイザ制御理論（Supervisory Control Theory, SCT）である~\cite{DiscreteControllerSynthesis, DiscreteEventSystem}．
SCTにおいて，制御対象である離散事象システム（DES）は形式言語の生成器としてモデル化される．
Ramadgeらは，システムが生成しうる言語と，仕様として与えられた許容言語との包含関係に基づき，スーパバイザの存在条件を記述した~\cite{DiscreteControllerSynthesis}．
彼らは，システムに対して禁止可能なイベントを動的に無効化することで，仕様を満たす最大の振る舞いである最大制御可能部分言語を合成できることを示した．
これにより，SafetyおよびSCTにおけるNon-Blockingを数学的に保証するスーパバイザの自動生成が可能となった．

しかし，システムが大規模化あるいは分散化するにつれて，単一の集中型スーパバイザを合成することは，計算量および実装の観点から困難となる．
これは状態空間爆発と呼ばれる問題である．
これに対し，LinとWonhamは，システムを複数の部分観測可能な局所スーパバイザによって制御する分散制御（Decentralized Control）の枠組みを提案した~\cite{DecentralizedSupervisoryControlOfDiscreteEventSystems}．
分散制御では，各局所スーパバイザが部分的な情報のみに基づいて制御判断を行い，それらの制御判断を融合することで，全体としてのSafetyを実現する．

また，大規模なシステムを効率的に扱うための階層的なアプローチも多数提案されている．
CaiとWonhamは，全体として合成された全体スーパバイザを，各エージェントが持つべき局所制御器へと分解するスーパバイザ・ローカリゼーション（Supervisor Localization）の手法を提案した~\cite{SupervisorLocalization}．
この手法はトップダウンのアプローチにより，大域的な制御仕様を等価な局所制御ロジックへと配分することを可能にする．
一方，Komendaらは，システムを複数のサブシステムとそれらを調停するコーディネータの組み合わせとして捉えるコーディネーションスキーム（Coordination Scheme）を提案した~\cite{SupervisoryControlSynthesisOfDiscreteEventSystemsUsingACoordinationScheme}．
彼らは，条件付き制御可能性という概念を導入し，コーディネータを用いることで分散的な合成を行いながらも，大域的な仕様を満たす制御器を構成できることを示した．

これらの研究は，DESの制御においてSafetyとNon-Blockingを保証するための堅牢な理論的基盤を提供している．
しかし，全体スーパバイザを事前に構築する手法や，複雑な階層構造を前提とする手法は，依然として状態空間爆発の問題や設計の複雑さに直面する場合がある．
この計算量的課題に対処するため，大別して静的なモデル縮約と動的な探索空間の限定という2つのアプローチが研究されてきた．

\section{静的なモデル縮約によるアプローチ}

状態空間爆発に対する第一のアプローチは，合成処理を行う前に，あるいは合成の過程でモデル自体を簡略化もしくは圧縮することで，計算対象となる状態空間そのものを削減する手法である．

WonhamとRamadgeは，システムのモジュール構造に着目し，モジュラー制御（Modular Supervisory Control）を提案した~\cite{ModularSupervisoryControlOfDiscreteEventSystems}．
本手法は，単一の巨大な全体仕様を複数の部分仕様に分割し，各仕様に対する制御器を個別に合成することで，全体の同期積を直接構成することを回避し，合成において扱う状態空間の増大を抑制する．
また，WongとWonhamは，モジュラー制御において局所的な制御器間の競合により発生するブロッキングを解決するため，コーディネータを用いた階層的な調停機構を導入した~\cite{ModularControlAndCoordinationOfDiscreteEventSystems}．

これらの構造的アプローチを発展させたものとして，構成的合成（Compositional Synthesis）がある．
Malikらは，構成的手法に関する包括的な調査を行い，コンポーネントごとの合成と抽象化を交互に繰り返すことで，全体モデルの構築を回避しつつ制御器を生成する手法群を体系化している~\cite{ASurveyOnCompositionalAlgorithmsForVerificationAndSynthesisInSupervisoryControl}．
具体的には，Mohajeraniらは，合成の各段階において制御に必要な情報を保持しつつモデルを簡略化する合成抽象化（Synthesis Abstraction）の概念を導入し，最小制限性を保持したまま大規模システムに対する合成を可能とする枠組みを示した~\cite{CompositionalSynthesisOfDiscreteEventSystemsUsingSynthesisAbstraction}．
また，山内らは，要求仕様に基づいて必要最小限のモデル構造を段階的に構築しつつ部分合成を行う手法（Stepwise Partial Synthesis）を提案し，充足不可能な状態を早期に剪定することで分析空間を削減している~\cite{StepwisePartialSynthesis}．

一方，状態空間の表現形式や構築プロセス自体を工夫することで計算資源を節約するアプローチも存在する．
Thuijsmanらは，二分決定グラフ（BDD）を用いたシンボリックな合成手法において，変数の順序付けや演算順序を最適化するヒューリスティックを提案し，明示的な状態列挙を行わずに大規模な論理空間を効率的に処理する手法を示した~\cite{ReducingTheComputationalEffortOfSymbolicSupervisorSynthesis}．
また，山内らは，環境モデルと監視モデルから全体モデルを構築することなく，ゲーム空間を直接構築するConsolidated Discrete Controller Synthesis（CDCS）を提案した~\cite{ConsolidatedDiscreteControllerSynthesis}．
CDCSは，安全性要件に違反する状態が生成されないよう制御ゲーム空間を構成することにより，大幅な計算空間削減を達成している．

これらの手法は，モデルの構造や定義を工夫することで計算対象となる空間自体を削減する有効なアプローチである．
しかし，これらの手法は，構造分割，抽象化，あるいはモデル表現の工夫といった設計時の前処理を前提とする場合が多く，未知の環境に対して動的に経路を決定するような柔軟性には欠ける側面がある．

\section{On-The-Fly探索による動的な空間削減}

第二のアプローチは，全状態空間を事前に構築することなく，初期状態から到達可能な必要な部分のみを動的に展開するOn-The-Fly探索である．
この手法は，特にシステムが巨大で全状態の列挙が不可能な場合や，解となる制御器が存在するかどうかの判定を早期に行いたい場合に有効である．

Cassezら~\cite{OnTheFlyForTimedGames}は，時間制約を含むTimed Gamesにおいて，従来主流であった後方固定点計算に基づく解法ではなく，前方探索により状態空間を動的に展開するOn-The-Flyアルゴリズムを提案した．
この手法では，勝利条件が満たされた時点で探索を終了できるため，全状態空間を構築することなく効率的な検証が可能であることを示している．
このアプローチは，システムが複雑化した現在においても状態空間爆発を抑制する有効な手段として重要視されており，Dahlsen-Jensenら~\cite{OnTheFlyForReachability}は，Parametric Timed Gamesにおける到達可能性問題およびパラメータ合成問題に対して，On-The-Fly探索を用いた解法を提案している．

制御器合成の分野においても，このOn-The-Fly探索の概念は積極的に導入されている．
Miremadiら~\cite{SymbolicOnTheFlySynthesis}は，SCTにおいてBDDを用いたシンボリックな状態表現とOn-The-Fly探索を組み合わせた合成手法を提案した．
彼らは，AIプランニングの探索技術に着想を得た前方探索を用いることで，制御不能状態やブロッキング状態を早期に検出し，不要な探索を抑制することで，大規模システムに対する合成の実用性を示している．

本研究が対象とするDirected Controller Synthesis（DCS）において，Ciolekらが提案したアルゴリズム~\cite{DirectedControllerSynthesis}もこの系譜に属す．
DCSは，各状態で高々1つの制御アクションを選択するという性質を持つため，On-The-Fly探索との親和性が高く，必要な制御パスのみを探索的に発見することで，完全な制御器合成を回避している．
このOn-The-Fly探索において最も重要な要素は，次にどの遷移を展開すべきかを決定する探索の指針，すなわちヒューリスティック関数の設計である．
適切なヒューリスティックが存在すれば，探索は解に向かって直線的に進むが，不適切な場合は全探索に近いコストを要することになる．

\section{DCSにおけるヒューリスティック探索}

DCSの効率化を目的としたヒューリスティックとして，現在最も有力な手法がCiolekらによるReady Abstraction（RA）~\cite{ReadyAbstraction}である．
RAは，各コンポーネントにおける局所的な遷移可能性であるReadyアクションと，アクション間の依存関係に基づいて目標までの距離を推定する手法である．
RAはドメイン独立な手法でありながら，並列合成構造を利用して多項式時間で計算可能であり，多くのベンチマーク問題で高い性能を示している．
具体的には，各コンポーネントの状態遷移グラフ上で，現在のアクションから目標アクションまでの遷移コストGapと到達距離Distを再帰的に計算することで，大域的な距離を見積もる．
RAの導入により，DCSの適用範囲は拡大し，従来の手法では扱えなかった規模のシステムに対しても制御器の合成が可能となったことが報告されている．

また，近年では人手によって設計されたヒューリスティックに加え，機械学習を用いて探索戦略を自動的に獲得する試みもなされている．
Delgadoらは，強化学習をDCSの探索プロセスに適用し，遷移展開の順序を決定する探索ポリシーを学習する手法を提案した~\cite{ReinforcementOnTheFly}．
彼らは，探索過程の状態や選択可能な遷移を特徴量空間に抽象化し，探索に要するステップ数を最小化するような方策をニューラルネットワークにより学習させた．
この手法は，小規模な問題インスタンスで学習したモデルを大規模なインスタンスへ適用するゼロショット転移によって，特定の問題設定においてはRAを上回る探索効率を実現できることを示している．

しかし，これらのDCS特化型ヒューリスティックには大域的な構造の看過という課題がある．
RAは局所的なReadyアクションの連鎖を見るため，将来的な同期待ちやデッドロックといった大域的なボトルネックを予測しきれない．
その結果，局所的には最適に見えるが実際には目標に到達できない経路を過剰に探索してしまうという問題が生じうる．
また，強化学習に基づく手法は，探索ポリシーの獲得に多大なトレーニングコストを要する上，学習データの質や問題の構造に性能が依存するため，汎用性に課題が残る．
より複雑な同期構造を持つシステムを扱うためには，局所的な遷移だけでなく，ゴールに至るために通過必須な要所を大域的に捉える視点が必要となる．

\section{AIプランニングにおけるランドマーク技術}

本研究で提案したLandmark Ready Abstractionは，AIプランニングの分野で発展したランドマーク（Landmark）の概念を，離散制御器合成の領域へと応用したものである．
AIプランニングは，初期状態から目標状態に至るアクションの列（プラン）を探索する問題であり，グラフ上の探索問題に帰着されるという点で，DCSと構造的な類似性を持つ．
プランニングの分野においても，問題の大規模化に伴う探索空間の指数的な増大は主要な課題であり，これに対処するためにドメイン独立な強力なヒューリスティック関数が多数提案されてきた．

特に，目標達成のために必ず達成しなければならないファクトや実行しなければならないアクションをランドマークとして定義し，これらを探索の道標として利用する手法は，探索効率を劇的に向上させる技術として確立されている．
DCSにおけるReady Abstractionが局所的な遷移可能性に着目するのに対し，ランドマークによるアプローチは大域的なボトルネック構造に着目する点に特徴がある．

Hoffmannらは，ランドマークをすべての妥当なプランにおいて必ず真にならなければならない事実として形式的に定義した~\cite{LandmarksInPlanning}．
彼らは，与えられた問題のゴール状態から逆方向に探索を行い，ある事実を達成するために必要な前提条件を連鎖的に特定することで，ランドマークおよびそれらの間の順序制約を抽出するアルゴリズムを提案した．
この手法により抽出されたランドマークグラフは，本来の探索問題をより小さなサブゴールへの到達問題へと分解するために用いられ，大規模なプランニング問題の解決に寄与した．

ランドマークを探索の分解だけでなく，ヒューリスティック関数の評価値として直接利用する手法も発展している．
Richterらが開発したLAMAプランナーは，未達成のランドマークの数をゴールまでの距離推定値として用いるLandmark Count Heuristicを採用し，国際プランニング競技会において優れた性能を示した~\cite{LAMAPlanner}．
LAMAのアプローチは，ランドマークを単なる通過点としてではなく，探索の進行度を測る定量的な指標として扱った点に革新性がある．
また，Karpasらは，ランドマークの概念をコスト最適プランニングへと拡張し，アクションコストが不均一な場合においても，最適解（最小コスト解）を発見するための許容的（Admissible）なヒューリスティック関数を構成できることを示した~\cite{CostOptimalPlanningWithLandmarks}．
これは，最短経路探索としての側面を持つDCSにおいて，ランドマーク情報が探索の誘導だけでなく解の品質保証にも寄与しうることを示唆している．

さらに近年では，Büchnerらがランドマーク間の依存関係における循環構造（Cyclic Dependencies）に着目し，サイクル内のランドマークが複数回達成される必要性を考慮した高精度なヒューリスティックを提案するなど，ランドマーク技術は現在も発展を続けている~\cite{CyclicLandmarkHeuristic}．

本研究におけるLandmark Ready Abstractionは，これらのAIプランニングにおけるランドマークの概念を，並列合成される離散事象システムのアクション構造へと適合させたものである．
プランニングが主に静的な状態遷移を扱うのに対し，DCSはコンポーネント間の同期や非同期動作といった動的な制約を扱う必要がある．
本研究で定義したPre-Markingアクションは，Hoffmannらの定義する必須事実に相当する概念を，DCSにおけるアクション単位のボトルネックとして再定義し，Karpasらの手法と同様にコスト（ステップ数）ベースのヒューリスティック関数へと統合したものである．
これにより，LRAはAIプランニングで実証された大域的な構造解析の利点を，離散制御器合成という異なるドメインにおいて実現している．
...
本論文では，離散事象システムの制御器合成技術であるDirected Controller Synthesis（DCS）において，探索の効率化を実現する新たなヒューリスティック手法Landmark Ready Abstraction（LRA）を提案した．

従来のReady Abstraction（RA）は，並列合成構造に基づいた局所的な遷移可能性を利用して目標までの距離を推定する強力な手法である．
しかし，\aref{chap:ready_abstraction_limitations}の分析を通じて，RAには同期構造の看過という構造的な課題があることを明らかにした．
RAは，各コンポーネントにおける局所的な最短経路を優先して評価するため，将来的な同期待ちやボトルネックとなる工程を適切に予測できず，結果として冗長な探索空間を拡大させる傾向がある．
また，すべてのコンポーネントに対して毎ステップ網羅的な距離推定を行う計算構造は，システム規模の拡大に伴い莫大な計算コストを要するという課題も存在した．

これらの課題を解決するため，\aref{chap:landmark_ready_abstraction}において，最終目標に至るために構造上回避不可能なアクションをPre-Markingアクション（PMA）として定義し，これを目標達成の中間目標（ランドマーク）として導入するLRAを提案した．
LRAは，静的解析によって抽出されたPMAを距離推定の指針に組み込むことで，同期ボトルネックを考慮した適切な経路選択を可能にする．
さらに，距離推定の実行対象を解決すべきPMAに厳選することで，1ステップあたりの計算負荷を抑制するアルゴリズムを構築した．

\aref{chap:evaluation}における評価実験の結果，提案手法LRAは以下の有効性を示した．
第一に，複雑な同期構造を持つシナリオにおいて，RAやSRAと比較して探索状態数を大幅に削減することに成功した．
特に，従来のヒューリスティックでは局所解への停留が発生していた問題において，PMAによる大域的な方向付けが探索の収束を劇的に早めることを確認した．
第二に，コンポーネント数が増大した大規模なシステムにおいて，計算時間の増大を抑制し，従来手法と比較して最大で300倍以上の高速化を達成した．
これにより，PMAの占有率が高いシステムにおいて，LRAが極めて高いスケーラビリティを有することを実証した．

以上の成果により，本研究が提案したLandmark Ready Abstractionは，大規模かつ複雑な同期制約を持つ離散事象システムに対して，高信頼な制御器を現実的な時間・メモリリソースで合成するための有効な手段であることを明らかにした．
...
本研究の遂行にあたり，多大なるご指導を賜りました東京科学大学情報理工学院の鄭顕志准教授に深く感謝いたします．
鄭先生には，研究の方向性に関する本質的な示唆をいただくとともに，論文の構成や論理的な記述の不備についても丁寧にご指摘をいただきました．
先生の真摯な研究姿勢から学んだことは多く，本論文を完成させる上での大きな糧となりました．

また，本研究の詳細な議論において，多大なるご支援をいただきました早稲田大学理工学術院の山内拓人講師に深く謝意を表します．
山内先生には，提案手法のアルゴリズム構築から実装，評価実験に至るまで，極めて具体的かつ専門的な観点から数多くのご指導をいただきました．
先生との議論を通じて得られた知見は，本研究の成果を洗練させる上で不可欠なものでした．

鄭研究室の諸氏には，日々の議論や研究生活を通じて有意義な時間を共有していただいたことに感謝いたします．
恵まれた研究環境の中で切磋琢磨し，互いに高め合えた経験は，筆者にとって何物にも代えがたい財産となりました．

最後に，大学院進学を認め，長期間にわたり精神的および経済的に支えてくれた家族に，心より感謝の意を表します．
