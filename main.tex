...
\section{AIプランニングにおけるLandmark Heuristic}

本研究で提案したLandmark Ready Abstractionは，AIプランニングの分野で発展したLandmark（ランドマーク）の概念を，離散制御器合成の領域へと応用したものである．
...
特に，目標達成のために必ず達成しなければならない事実（Fact）や実行しなければならないアクション（Action）をLandmarkとして定義し，これらを探索の道標として利用する手法は，探索効率を劇的に向上させる技術として確立されている．
DCSにおけるReady Abstractionが局所的な遷移可能性に着目するのに対し，Landmarkによるアプローチは大域的なボトルネック構造に着目する点に特徴がある．

Hoffmannらは，Landmarkをすべての妥当なプランにおいて必ず真にならなければならない事実として形式的に定義した~\cite{LandmarksInPlanning}．
彼らは，与えられた問題のゴール状態から逆方向に探索を行い，ある事実を達成するために必要な前提条件を連鎖的に特定することで，Landmarkおよびそれらの間の順序制約を抽出するアルゴリズムを提案した．
この手法により抽出されたLandmarkグラフは，本来の探索問題をより小さなサブゴールへの到達問題へと分解するために用いられ，大規模なプランニング問題の解決に寄与した．

Landmarkを探索の分解だけでなく，ヒューリスティック関数の評価値として直接利用する手法も発展している．
Richterらが開発したLAMAプランナーは，未達成のLandmarkの数をゴールまでの距離推定値として用いるLandmark Count Heuristicを採用し，国際プランニング競技会において優れた性能を示した~\cite{LAMAPlanner}．
LAMAのアプローチは，Landmarkを単なる通過点としてではなく，探索の進行度を測る定量的な指標として扱った点に革新性がある．
また，Karpasらは，Landmarkの概念をコスト最適プランニングへと拡張し，アクションコストが不均一な場合においても，最適解（最小コスト解）を発見するための許容的（Admissible）なヒューリスティック関数を構成できることを示した~\cite{CostOptimalPlanningWithLandmarks}．
これは，最短経路探索としての側面を持つDCSにおいて，Landmark情報が探索の誘導だけでなく解の品質保証にも寄与しうることを示唆している．

さらに近年では，BüchnerらがLandmark間の依存関係における循環構造（Cyclic Dependencies）に着目し，サイクル内のLandmarkが複数回達成される必要性を考慮した高精度なヒューリスティックを提案するなど，Landmark技術は現在も発展を続けている~\cite{CyclicLandmarkHeuristic}．

本研究におけるLandmark Ready Abstractionは，これらのAIプランニングにおけるLandmarkの概念を，並列合成される離散事象システムのアクション構造へと適合させたものである．
...
また，本研究の詳細な議論において，多大なるご支援をいただきました山内先生に深く謝意を表します．
